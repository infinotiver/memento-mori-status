...
function activate(context) {
  ...
  let displayFormat =
    config.get("displayFormat") ||
    "Day %dayProgress% · Year %yearProgress% · Life %lifeProgress%";

  // Validate configuration values
  function validateConfig() {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(birthDateStr)) {
      vscode.window.showErrorMessage(
        `Invalid birth date format: ${birthDateStr}. Resetting to default.`
      );
      birthDateStr = "1990-01-01";
    }
    if (isNaN(lifeExpectancy) || lifeExpectancy <= 0) {
      vscode.window.showErrorMessage(
        `Invalid life expectancy: ${lifeExpectancy}. Resetting to default.`
      );
      lifeExpectancy = 80;
    }
    if (typeof displayFormat !== "string") {
      vscode.window.showErrorMessage(
        `Invalid display format. Resetting to default.`
      );
      displayFormat =
        "Day %dayProgress% · Year %yearProgress% · Life %lifeProgress%";
    }
  ...
  function updateStatus() {
    ...
    const daysLeft = Math.max(
      0,
      Math.floor(
        (expectedDeath.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
      )
    );
    ...
    const daysToNextBirthday = Math.max(
      0,
      Math.floor(
    ...
    const createProgressBar = (progress) => {
      const value = Math.min(
        10,
        Math.max(0, Math.round(parseInt(progress, 10) / 10))
      );
  ...
  const setBirthDate = vscode.commands.registerCommand(
    "mementoMori.setBirthDate",
    async () => {
      ...
      if (input) {
        await vscode.workspace
          .getConfiguration("mementoMori")
          .update("birthDate", input, vscode.ConfigurationTarget.Global);
        birthDateStr = input;
        updateStatus();
  ...
  const resetConfig = vscode.commands.registerCommand(
    "mementoMori.resetConfig",
    async () => {
      await vscode.workspace
        .getConfiguration("mementoMori")
        .update("birthDate", "1990-01-01", vscode.ConfigurationTarget.Global);
      await vscode.workspace
        .getConfiguration("mementoMori")
        .update("lifeExpectancy", 80, vscode.ConfigurationTarget.Global);
      await vscode.workspace
        .getConfiguration("mementoMori")
        .update(
          "displayFormat",
